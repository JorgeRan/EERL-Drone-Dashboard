...
const buildAerisSnapshotPoint = (timestamp, sensorMode, state) => {
  ...
  const optionalFields = {
    ...
    methane_valid: state.methane_valid,
    methanePressureMbar: state.methanePressureMbar,
    methanePowerInputMv: state.methanePowerInputMv,
    nitrousOxidePressureMbar: state.nitrousOxidePressureMbar,
    nitrousOxidePowerInputMv: state.nitrousOxidePowerInputMv,
    acetylenePressureMbar: state.acetylenePressureMbar,
    acetylenePowerInputMv: state.acetylenePowerInputMv,
    nitrousOxideValid: state.nitrousOxideValid,
    acetyleneValid: state.acetyleneValid,
...
const parseAerisCsvToMissionResults = (
  ...
  const flightStatusIdx = findHeaderIndex(
    headers,
    (header) =>
      header === "flight_status" || header.endsWith(".flight_status"),
  );
  const methaneValidIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("ch4_valid")
    : -1;
  const methanePressureIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("ch4_pressure_mbar")
    : -1;
  const methanePowerIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("ch4_power_input_mv")
    : -1;
  const nitrousOxidePressureIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("n2o_pressure_mbar")
    : -1;
  const nitrousOxidePowerIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("n2o_power_input_mv")
    : -1;
  const nitrousOxideValidIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("n2o_valid")
    : -1;
  const acetylenePressureIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("c2h2_pressure_mbar")
    : -1;
  const acetylenePowerIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("c2h2_power_input_mv")
    : -1;
  const acetyleneValidIdx = isDirectAerisFormat
    ? normalizedHeaders.indexOf("c2h2_valid")
    : -1;
  ...
  const rollingState = {
    ...
    nitrousOxide: null,
    methane_valid: null,
    methanePressureMbar: null,
    methanePowerInputMv: null,
    nitrousOxidePressureMbar: null,
    nitrousOxidePowerInputMv: null,
    nitrousOxideValid: null,
    acetylenePressureMbar: null,
    acetylenePowerInputMv: null,
    acetyleneValid: null,
    latitude: null,
    longitude: null,
    altitude: null,
  ...
  for (const line of lines.slice(1)) {
    ...
    const updates = {
      ...
      methanePressureMbar:
        methanePressureIdx !== -1 ? parseNumber(cols[methanePressureIdx]) : null,
      methanePowerInputMv:
        methanePowerIdx !== -1 ? parseNumber(cols[methanePowerIdx]) : null,
      nitrousOxidePressureMbar:
        nitrousOxidePressureIdx !== -1
          ? parseNumber(cols[nitrousOxidePressureIdx])
          : null,
      nitrousOxidePowerInputMv:
        nitrousOxidePowerIdx !== -1
          ? parseNumber(cols[nitrousOxidePowerIdx])
          : null,
      nitrousOxideValid:
        nitrousOxideValidIdx !== -1
          ? parseNumber(cols[nitrousOxideValidIdx])
          : null,
      acetylenePressureMbar:
        acetylenePressureIdx !== -1
          ? parseNumber(cols[acetylenePressureIdx])
          : null,
      acetylenePowerInputMv:
        acetylenePowerIdx !== -1
          ? parseNumber(cols[acetylenePowerIdx])
          : null,
      acetyleneValid:
        acetyleneValidIdx !== -1 ? parseNumber(cols[acetyleneValidIdx]) : null,
